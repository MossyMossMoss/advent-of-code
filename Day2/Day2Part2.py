...
def get_string_num(string, start):
    output = ""
    index = start
...
for a in Input:
    charone = get_string_num(a, 0)
    chartwo = get_string_num(a , charone[1] + 1)
    char = a[chartwo[1] + 1]
    charpresent = False
    password = a[(chartwo[1] + 4):]
    if xor(password[charone[0] - 1] == char, password[chartwo[0] - 1] == char):
        print(a)
        charpresent = True
    if charpresent == True:
        Valid += 1
